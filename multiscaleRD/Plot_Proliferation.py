...
for i in range(numSimulations):
    PreySimulation=np.load('./Simulations/Particles'+str(i)+'.npy', allow_pickle=True, encoding='latin1')
    for j in range(len(PreySimulation)):
        AllSimulations.append(PreySimulation[j])


MeanField=functionAverage(AllSimulations)
np.save('./Solutions/MeanField', MeanField)
...
MeanField=np.load('./Solutions/MeanField.npy')
Reference=np.load('./Solutions/Reference.npy')
FDSolution=np.load('./Solutions/FDSolution.npy')
...
functionplot(Hybrid[int(time[1])], 6, np.round(time[1]*dt,2), 'Hybrid')
functionplot(Reference[int(time[1])], 6, np.round(time[1]*dt,2), 'Reference')

functionplot(Hybrid[int(time[2])], 6, np.round(time[2]*dt,2), 'Hybrid')
functionplot(Reference[int(time[2])], 6, np.round(time[2]*dt,2), 'Reference')

functionplot(Hybrid[int(time[3])], 6, np.round(time[3]*dt,2), 'Hybrid')
functionplot(Reference[int(time[3])], 6, np.round(time[3]*dt,2), 'Reference')
